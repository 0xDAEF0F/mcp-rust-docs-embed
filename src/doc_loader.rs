use crate::{docs_builder::build_crate_docs, json_types::JsonDocs, my_types::DocItem};
...
/// Returns the DocItems, resolved version, and temp directory (for accessing source
/// files).
pub fn load_documents(
	crate_name: &str,
	crate_version_req: &str,
	features: &[String],
) -> Result<(Vec<DocItem>, String, TempDir)> {
	...
	let doc_items: Vec<DocItem> = (&rustdoc).into();
	...
	Ok((doc_items, resolved_version, temp_dir))
}
